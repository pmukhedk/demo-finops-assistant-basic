...
st.markdown("""
Upload supported billing data sources, including AWS Cost and Usage Reports, Azure Cost Exports, and CSV files, to enable detailed cost analysis aligned with FinOps principles and natural language–based insight discovery.
""")
...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        if f"processed_{file.name}" not in st.session_state:
            with st.spinner(f"Processing {file.name}..."):
                ...
                if not text:
                    st.sidebar.warning(f"❌ Unsupported or empty file: {file.name}")
                    continue
                ...
                if df is not None:
                    with st.spinner("🤖 Generating FinOps Health Assessment..."):
                        ...
                        TONE: Professional, critical, and directive. Use emojis (🔹, 📌, ✅, ⚠️, ❌) for readability.
                        """
    ...
    for file in uploaded_files:
        if f"analysis_{file.name}" in st.session_state:
            ...
            st.markdown("---")
            st.subheader(f"📊 FinOps Analysis: {file.name}")
...
if 'analysis_exists' in locals() and analysis_exists:
    ...
    if question:
        ...
        if not docs:
            ...
            with st.expander("🕵️ Debug: What the AI sees (Context)"):
                st.text(context)
...
"""
            ...
            st.markdown("### 💡 FinOps Insight")
            st.write(response.choices[0].message.content)

            with st.expander("📊 Source Data (Context)"):
                st.code(context)
